...
EventEmitter.prototype.removeListener = function(event, listener) {
	...
	if(isArray(eventListeners) && eventListeners.length && (index = eventListeners.indexOf(listener))) {
		 ...
		 eventListeners.indexOf(listener) === -1 && this.emit(event, listener);
	}

	if(isArray(onceListeners) && onceListeners.length && (index = onceListeners.indexOf(listener))) {
		 onceListeners.splice(index, 1);
		 onceListeners.indexOf(listener) === -1 && this.emit(event, listener);
	}
...
EventEmitter.prototype.removeAllListeners = function(event) {
	...
	function __removeLE(eve, listeners) {
		...
		while(listeners.length) {
			...
			listeners.indexOf(listener) === -1 && emiter.emit(eve, listener);
		}
	}
